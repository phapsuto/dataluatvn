...
*   **Mã nguồn máy chủ API:** [server.py](file:///Users/tonguyen/Library/CloudStorage/OneDrive-Personal/DrTo/luatvietnam/server.py) (Viết bằng FastAPI, chạy ngầm tại cổng `2004`).
...
Mặc định máy chủ đang chạy ngầm trên máy của bạn tại cổng `2004`. Nếu cần khởi động lại thủ công, bạn mở Terminal tại thư mục này và gõ:
```bash
python3 server.py
```
> [!TIP]
> Bạn có thể xem tài liệu tương tác trực quan (Swagger UI), thử nghiệm gọi API trực tiếp ngay trên trình duyệt tại địa chỉ: **[http://localhost:2004/docs](http://localhost:2004/docs)**.
...
def get_law_detail(law_id):
    url = f"http://localhost:2004/laws/{law_id}"
    try:
...
## 🔌 PHƯƠNG THỨC 3: KẾT NỐI QUA MCP SERVER (Dành Cho Cursor / Claude Desktop)

Phương thức này được phát triển đặc biệt để tích hợp trực tiếp cơ sở dữ liệu pháp luật với các AI Editor/AI Chatbot cục bộ như **Cursor** và **Claude Desktop**. 

### 1. Nguyên lý hoạt động
Tệp [mcp_server.py](file:///Users/tonguyen/Library/CloudStorage/OneDrive-Personal/DrTo/luatvietnam/mcp_server.py) chạy như một ứng dụng nền giao tiếp thông qua luồng vào/ra tiêu chuẩn (`stdin` / `stdout`). Khi AI cần truy vấn luật, nó sẽ tự động gọi các công cụ (Tools) được định nghĩa sẵn để đọc CSDL.

### 2. Các công cụ hỗ trợ
*   `search_laws`: Tra cứu FTS5 theo từ khóa, lọc theo loại văn bản và tình trạng hiệu lực.
*   `get_document`: Lấy siêu dữ liệu và **nội dung văn bản thô đã lọc sạch thẻ HTML** giúp tiết kiệm ngữ cảnh (context window) cho LLM.
*   `get_law_lineage`: Trích xuất cây phả hệ pháp luật liên kết (văn bản căn cứ, sửa đổi, thay thế).
*   `check_law_overlaps`: Tự động đối soát quy định chồng chéo theo Điều 156 Luật ban hành VBQPPL.

### 3. Cấu hình chi tiết
1.  **Với Cursor**: Vào `Settings` -> `Models` -> `MCP` -> Chọn `Add New MCP Server`.
    *   *Name*: `dataluatvn-mcp`
    *   *Type*: `stdio`
    *   *Command*: `python3 /Users/tonguyen/Library/CloudStorage/OneDrive-Personal/DrTo/luatvietnam/mcp_server.py`
2.  **Với Claude Desktop**: Mở file cấu hình tại `~/Library/Application Support/Claude/claude_desktop_config.json` và thêm cấu hình server `dataluatvn-mcp` (xem chi tiết tại tệp `README.md`).
...
[Trích xuất từ khóa hoặc Tạo Vector nhúng của câu hỏi]
       ↓
[Gọi API Search: http://localhost:2004/laws/search?q=từ_khóa&status=Còn+hiệu+lực]
       ↓
[Lấy ra top 3 văn bản phù hợp nhất]
       ↓
[Gọi API Detail: http://localhost:2004/laws/{id} để lấy toàn văn HTML/chữ sạch của 3 luật đó]
       ↓
