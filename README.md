...
*   🥤 **TokenJuice HTML Parser:** Bộ parser tinh giản làm sạch HTML thô từ web và chuẩn hóa cấu trúc văn bản thành Markdown có phân cấp rõ ràng (`# Điều 1`, `* Khoản 1`). Giúp mô hình Embedding (BGE-M3) nắm bắt ngữ cảnh tốt nhất, tối đa hóa chỉ số Recall khi tìm kiếm ngữ nghĩa.
*   🔄 **Cơ Chế Đồng Bộ Gia Tăng (Incremental Auto-fetch):** Tự động đồng bộ các văn bản luật mới hàng ngày và cập nhật gia tăng (append) vào SQLite và cả 3 chỉ mục FAISS trên đĩa (Flat, SQ8, IVF-SQ8) mà không cần xây dựng lại toàn bộ chỉ mục từ đầu.
...
## 🔄 Tự Động Đồng Bộ Luật Mới & Cập Nhật FAISS Gia Tăng

Hệ thống được trang bị bộ cập nhật luật tự động hàng ngày:
```bash
python3 scripts/sync_new_laws.py
```
*Lưu ý:* Tập lệnh này tự động thực hiện các bước sau:
1. Quét và tải các văn bản luật mới phát sinh.
2. Dùng bộ parser **TokenJuice-style** (`html_to_clean_markdown` trong `scratch/build_chunks_v2.py`) để loại bỏ code CSS/HTML rác và chuẩn hóa văn bản sang cấu trúc Markdown.
3. Sinh vector nhúng BGE-M3 (chạy offline) và lưu cache vào `vector_store.db`.
4. Cập nhật gia tăng (append) các vector mới trực tiếp vào cả 3 tệp chỉ mục FAISS trên đĩa (`chunks_faiss.index`, `chunks_faiss_sq8.index`, và `chunks_faiss_ivf_sq8.index` sử dụng `add_with_ids`) để tránh phải tính toán build lại toàn bộ từ đầu.
5. Cập nhật trạng thái văn bản cũ và trích xuất thực thể đồ thị tri thức mới đưa vào LightGraph Store cục bộ.
